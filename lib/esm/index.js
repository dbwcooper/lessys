...
var getFileUTF8 = async (lessPath) => fs.readFile(lessPath, "utf8");
...
var lessToCss = async (lessInputStr, themeItem) => {
  ...
  return less.render(lessInputStr, renderOptions).then((output) => output.css.replace(/:global ?/g, "")).then(removeComments).catch((e) => {
    console.log(e);
    return "";
  });
};
...
var mergeCss = (commonThemeList, config, watchingLessPath) => {
  return Promise.all(commonThemeList.map((item) => {
    const regx = path2.resolve(config.componentDir + "/**/" + item.cateKey + "/" + item.outputCssName);
    gulp.src(regx).pipe(gulpConcat(item.outputCssName)).pipe(gulp.dest(path2.parse(item.outputCssPath).dir));
...
var getSingleTheme = async (data) => {
  ...
  const cssName = path3.parse(data.themePath).name + ".css";
  const lessName = path3.parse(data.themePath).name + ".less";
  const outputCssPath = path3.join(data.outputDir, data.cateKey + "/" + cssName);
  const originLessPath = path3.resolve(data.themePath);
  return __spreadValues({
    originLessPath,
    outputCssPath: path3.resolve(outputCssPath),
...
var generateOneLess = async (commonThemeList, config, lessFilePath) => {
  const outputcomponentDirPath = path3.resolve(config.outputDir, "monitor");
  const componentDirPath = path3.resolve(config.componentDir);
  lessFilePath = path3.resolve(lessFilePath);
  return getFileUTF8(lessFilePath).then((lessStr) => {
    const pathParser = path3.parse(lessFilePath);
    ...
    return Promise.all(commonThemeList.map((item) => {
      return extractLess(lessStr, item.lessVariables, item.lessFunction.funcDefined).then(async (outputLessStr) => {
        const outputLessPath = path3.join(pathParser.dir.replace(componentDirPath, outputcomponentDirPath), item.cateKey, item.outputLessName);
        const outputCssPath = path3.join(pathParser.dir.replace(componentDirPath, outputcomponentDirPath), item.cateKey, item.outputCssName);
        ...
        await Promise.all([
          fs2.createFile(outputLessPath).then(() => fs2.writeFile(outputLessPath, outputLessStr)),
          fs2.createFile(outputCssPath).then(() => fs2.writeFile(outputCssPath, cssStr))
        ]);
        return result;
      });
    }));
  ...
  return Promise.all(lessPaths.map((lessPath) => generateOneLess(commonThemeList, config, lessPath))).then((results) => {
    ...
    return Promise.all([
      Object.keys(themeCssMap).map((themeCssPath) => {
        return fs2.createFile(themeCssPath).then(() => fs2.writeFile(themeCssPath, themeCssMap[themeCssPath]));
      })
    ]).then(() => "success");
  });
};
